...
To confirm the creation of namespaces and the bridge with its connected interfaces:



```bash

# List all created network namespaces
...
To verify IP address and routing inside a specific namespace (e.g., `nginx-lb`), after running `assignment1.sh`:



```bash

# Show the IP address on the veth-lb interface inside the nginx-lb namespace

sudo ip netns exec nginx-lb ip addr show veth-lb



# Show the routing table inside the nginx-lb namespace

sudo ip netns exec nginx-lb ip route
